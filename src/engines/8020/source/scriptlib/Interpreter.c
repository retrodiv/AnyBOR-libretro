 ...
 * Release parser sources, pack compiled instructions and their owned values
 * into exact contiguous storage, and dispatch the compact representation
 * through explicit target tags. Allocate parser state only while compiling
 * and release it once runtime instructions are ready.
...
HRESULT Interpreter_CompileInstructions(Interpreter *pinterpreter)
{
    ...
    for(i = 0; i < size; i++)
    {
        ...
        if(pInstruction->storageType == INSTRUCTION_SOURCE_TOKEN &&
           pInstruction->theToken)
        {
            free(pInstruction->theToken);
            pInstruction->theToken = NULL;
        }
        else if(pInstruction->storageType == INSTRUCTION_SOURCE_LABEL &&
                pInstruction->Label)
        {
    ...
    for(i = 0; i < size; i++)
    {
        ...
        if(pInstruction->jumpTargetType == INSTRUCTION_TARGET_INDEX &&
           pInstruction->theJumpTargetIndex >= 0 &&
